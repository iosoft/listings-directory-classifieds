...
class w2dc_locations_levels {
	...
	public function getLevelsFromDB() {
		...
		$array = $wpdb->get_results("SELECT * FROM wp_w2dc_locations_levels", ARRAY_A);
	...
	public function getSelectionsArray() {
		$selections = array();
		foreach ($this->levels_array AS $level)
			$selections[] = sprintf('Select %s', $level->name);
	...
	public function createLevelFromArray($array) {
		...
		return $wpdb->insert('wp_w2dc_locations_levels', $insert_update_args);
	}
	...
	public function saveLevelFromArray($level_id, $array) {
		...
		return $wpdb->update('wp_w2dc_locations_levels', $insert_update_args,	array('id' => $level_id), null, array('%d')) !== false;
	}
	
	public function deleteLevel($level_id) {
		global $wpdb;
	
		$wpdb->delete('wp_w2dc_locations_levels', array('id' => $level_id));
		return true;
	}
}
