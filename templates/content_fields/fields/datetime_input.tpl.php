<script>
	jQuery(document).ready(function($) {
		$("#w2dc_field_input_<?php echo $content_field->id; ?>").datepicker({
			showOn: "both",
			...
			onSelect: function(dateText) {
				var sDate = $("#w2dc_field_input_<?php echo $content_field->id; ?>").datepicker("getDate");
				if (sDate) {
				...
				$("input[name=w2dc_field_input_<?php echo $content_field->id; ?>]").val(tmstmp_str);
			}
		});
		<?php if ($content_field->value['date']): ?>
		$("#w2dc_field_input_<?php echo $content_field->id; ?>").datepicker('setDate', $.datepicker.parseDate('@', '<?php echo $content_field->value['date']; ?>000'));
		<?php endif; ?>
		$("#reset_date").click(function() {
			$.datepicker._clearDate('#w2dc_field_input_<?php echo $content_field->id; ?>');
		})
	});
</script>
<div class="w2dc_field w2dc_field_input_block w2dc_field_input_block_<?php echo $content_field->id; ?>">
	...
	<input type="text" id="w2dc_field_input_<?php echo $content_field->id; ?>" class="w2dc_field_input_datetime" />
	<input type="hidden" name="w2dc_field_input_<?php echo $content_field->id; ?>" value="<?php echo esc_attr($content_field->value['date']); ?>"/>
	<input type="button" class="button button-primary button-small" id="reset_date" value="<?php _e('reset date', 'W2DC')?>" />
	<?php if ($content_field->is_time): ?>
	&nbsp;&nbsp;&nbsp;<?php _e('Time:', 'W2DC'); ?>
	<select name="w2dc_field_input_hour_<?php echo $content_field->id; ?>">
	...
	&nbsp;:&nbsp;
	<select name="w2dc_field_input_minute_<?php echo $content_field->id; ?>">
