    ...
    jQuery(document).ready(function($) {
		...
		if (map_markers_attrs_array.length) {
				...
				if (document.getElementById("maps_canvas_"+unique_map_id)) {
				    ...
				    if (markers_array.length) {
						...
						if (markers_array.length == 1) {
							var zoom_level = markers_array[0][7];
						} else {
			...
			if (typeof radius_params != 'undefined') {
	...
	function placeMarker(glocation, unique_map_id) {
		...
		jQuery("#open_iw_" + glocation.listing_seo_title).click(function() {
			showInfoWindow(glocation, marker, maps[unique_map_id]);
	...
	function showInfoWindow(glocation, marker, map) {
		...
		if (glocation.listing_logo) {
			...
			windowHtml += '<img width="70px" class="map_window_logo" src="' + global_server_path + '/users_images/logos/' + glocation.listing_logo + '" />';
			if (glocation.listing_seo_title)
				windowHtml += '</a>';
		}
		if (glocation.listing_seo_title)
			windowHtml += '<a href="#post-' + glocation.listing_seo_title + '">';
		...
		if (glocation.listing_url || glocation.listing_seo_title)
			windowHtml += '<div><br />';
		if (glocation.listing_seo_title)
			windowHtml += '<b><a href="#post-' + glocation.listing_seo_title + '">' + view_summary_label + '</a></b>&nbsp;&nbsp;&nbsp;';
